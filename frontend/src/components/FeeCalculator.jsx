...
export default function FeeCalculator() {
  ...
  const sendMe = async (e) => {
    e.preventDefault();
    if (!name || !phone) { toast({ title: 'Add your name and phone to receive the estimate.' }); return; }
    try {
      setSending(true);
      await submitLead({
        name, phone,
      ...
      toast({ title: 'Sent!', description: `We’ll WhatsApp your ${c.name} estimate to ${phone} within a few minutes.` });
      setName(''); setPhone('');
    } catch (err) {
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-end">
          <div className="lg:col-span-7">
            <div className="flex items-center gap-2 text-[11px] mono uppercase tracking-widest text-coral">
              <Calculator className="h-3.5 w-3.5" /> / 04 — Live estimate
            </div>
            <h2 className="serif mt-3 text-5xl sm:text-6xl font-normal leading-[0.95] text-ink">Pick a country. Enter NEET.<br/><em className="font-light">See your fee & timeline.</em></h2>
          </div>
          <p className="lg:col-span-5 text-ink/70 text-[15px] leading-relaxed">
            A ball-park number in ten seconds — tuition, living, duration and your NEET-eligibility band. Fine-tune with a real counsellor after.
          </p>
        </div>
        ...
        <div className="mt-12 grid lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 rounded-3xl bg-white border border-ink/10 p-6 lg:p-8">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {countries.map((cc) => (
                  <button key={cc.code} onClick={() => setCode(cc.code)} className={`group text-left rounded-2xl border p-3 transition-all ${code===cc.code ? 'bg-ink text-cream border-ink' : 'bg-cream/60 border-ink/10 hover:border-ink/30'}`}>
                    ...
                    <div className={`text-[10px] mono uppercase tracking-widest ${code===cc.code ? 'text-cream/60' : 'text-ink/40'}`}>{cc.fee}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <form onSubmit={sendMe} className="mt-6 pt-6 border-t border-ink/10">
              <div className="text-[10px] mono uppercase tracking-widest text-coral">WhatsApp me this estimate</div>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" className="rounded-lg border border-ink/15 bg-cream/60 px-3 py-2.5 text-[13px] focus:border-forest"/>
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="WhatsApp number" className="rounded-lg border border-ink/15 bg-cream/60 px-3 py-2.5 text-[13px] focus:border-forest"/>
              </div>
              <button type="submit" disabled={sending} className="mt-3 w-full rounded-lg bg-coral hover:bg-[#d94a26] text-white font-semibold py-2.5 text-[13px] disabled:opacity-60 inline-flex items-center justify-center gap-2">
                {sending ? 'Sending…' : 'Send my estimate'} <ArrowUpRight className="h-4 w-4"/>
              </button>
            </form>
          </div>
            ...
            <div className="relative">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-[10px] mono uppercase tracking-widest text-coral">Personalised estimate</div>
                </div>
                <div className={`text-[10px] font-bold uppercase tracking-widest rounded-full px-2.5 py-1 ${elig.color}`}>{elig.tag}</div>
              </div>
              <div className="mt-4 flex items-baseline gap-3">
                <div className="serif text-6xl sm:text-7xl font-light leading-none">{c.name}</div>
              </div>
              <p className="mt-3 text-cream/70 text-[14px] max-w-lg">{elig.note}</p>

              <div className="mt-8 grid sm:grid-cols-3 gap-3">
                <div className="rounded-2xl bg-white/5 border border-cream/10 p-4">
                  <div className="text-[10px] mono uppercase tracking-widest text-cream/50 flex items-center gap-1"><IndianRupee className="h-3 w-3"/> Total (tuition + living)</div>
                  <div className="mt-1 serif text-3xl font-medium">₹{totalLow}–{totalHigh}L</div>
                  <div className="text-[11px] text-cream/60 mt-1">Over {c.years} years</div>
                </div>
                <div className="rounded-2xl bg-white/5 border border-cream/10 p-4">
                  <div className="text-[10px] mono uppercase tracking-widest text-cream/50 flex items-center gap-1"><GraduationCap className="h-3 w-3"/> Tuition / year</div>
                  <div className="mt-1 serif text-3xl font-medium">₹{perYearLow}–{perYearHigh}L</div>
                  <div className="text-[11px] text-cream/60 mt-1">Government universities</div>
                </div>
                <div className="rounded-2xl bg-white/5 border border-cream/10 p-4">
                  <div className="text-[10px] mono uppercase tracking-widest text-cream/50 flex items-center gap-1"><Clock className="h-3 w-3"/> Duration</div>
                  <div className="mt-1 serif text-3xl font-medium">{c.years} yrs</div>
                  <div className="text-[11px] text-cream/60 mt-1">MBBS · English-medium</div>
                </div>
              </div>

              <div className="mt-8">
                <div className="text-[10px] mono uppercase tracking-widest text-cream/50">Suggested timeline</div>
                <div className="mt-3 flex items-center gap-2 flex-wrap">
                  {['Free counselling', 'Doc prep (2–4 wks)', 'Admission letter', 'Visa & tickets', `Fly to ${c.name}`].map((s, i) => (
                    <React.Fragment key={s}>
                      <div className="rounded-full border border-cream/15 px-3 py-1.5 text-[12px] font-medium text-cream/90">{s}</div>
                      {i < 4 && <div className="h-px w-4 bg-cream/20" />}
                    </React.Fragment>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href={brand.applyLink} target="_blank" rel="noreferrer" className="group inline-flex items-center gap-2 rounded-full bg-coral hover:bg-[#d94a26] text-white px-5 py-3 text-[13px] font-bold">
                  Apply for {c.name} <ArrowUpRight className="h-4 w-4 transition-transform group-hover:rotate-45"/>
                </a>
                <a href={brand.callbackLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-cream/25 text-cream px-5 py-3 text-[13px] font-semibold hover:bg-cream/10">
                  Request callback
                </a>
              </div>
            </div>
