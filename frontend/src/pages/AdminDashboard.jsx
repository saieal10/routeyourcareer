...
export default function AdminDashboard() {
  ...
  const courseCountries = useMemo(() => {
    const countries = [
      ...courses.map(x => x.country).filter(Boolean),
      ...universities.map(x => x.country).filter(Boolean)
    ];
  ...
  const saveCourse = async status => {
    ...
    const payload = {
      ...
      // IMPORTANT:
      // For NEW courses, do not send a slug at all.
      // The backend generates a university-specific slug such as:
